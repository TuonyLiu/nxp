...
int  mjpegdec_decode(mjpeg_info_t *p_mjpeg, uint8_t *p_buf, uint32_t bsize)
{
    ...
    while (p_mjpeg->cinfo->output_scanline < p_mjpeg->cinfo->output_height) {
        ...
        buffer[0] = (JSAMPARRAY)(gp_lcd_addr + ((p_mjpeg->cinfo->output_scanline   +   p_mjpeg->imgoffy) * 800  + p_mjpeg->imgoffx) * 2);


        buffer[1] = (JSAMPARRAY)(gp_lcd_addr + ((p_mjpeg->cinfo->output_scanline + 1 + p_mjpeg->imgoffy) * 800  + p_mjpeg->imgoffx) * 2);
